...
import Database.Relational.Query
  (Table, Pi, id', Projection, Relation, ProductConstructor (..),
...
defineOverloadedLabels :: ConName                                      -- ^ Record type name
                       -> [((VarName, VarName, TypeQ), Maybe (TypeQ, VarName))] -- ^ Renamed column info list
                       -> Q [Dec]                                      -- ^ Column projection path declarations
defineOverloadedLabels recTypeName c1 =
  fmap join . sequenceA $ [ defineOverloadedLabel recTypeName n | n <- c1 ]
...
defineOverloadedLabel :: ConName                                    -- ^ Record type name
                      -> ((VarName, VarName, TypeQ), Maybe (TypeQ, VarName)) -- ^ Column info list
                      -> Q [Dec]                                    -- ^ Column projection path declarations
...
defineColumnsDefault :: ConName                          -- ^ Record type name
                     ...
                     -> NameConfig
                     -> Q [Dec]                          -- ^ Column projection path declarations
defineColumnsDefault recTypeName cols nmconfig = do
  ...
  if overloadedRecords nmconfig then do
    ...
    pure ds
  where
    columnInfoList = [((varN n, ct), fmap (withCName n) mayC) | ((n, ct), mayC) <- cols]
    overloadedRecordsColumnInfoList =
      [((varCamelcaseName n, varN n, ct), fmap (withCName n) mayC) | ((n, ct), mayC) <- cols]
    varN      name   = if overloadedRecords nmconfig then
                         (name ++ "'") `varNameWithPrefix` (nameBase $ conName recTypeName)
                       else
